...
export default function PartnerLogo({ name, className = "h-7 w-auto", logoUrl, officialLogoUrl }: PartnerLogoProps) {
  const [imageError, setImageError] = useState(false);

  let finalLogoUrl = officialLogoUrl || logoUrl;
  if (!finalLogoUrl) {
    ...
    if (matchedPartner) {
      finalLogoUrl = matchedPartner.officialLogoUrl || matchedPartner.logoUrl;
    }
  }
  ...
  return (
    <div 
      className={`inline-flex items-center justify-center px-4 py-2 rounded-xl bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-800 text-slate-800 dark:text-slate-200 font-sans font-extrabold text-sm tracking-wide shadow-sm transition-all duration-300 max-w-full overflow-hidden text-ellipsis whitespace-nowrap ${className}`}
      title={name}
      role="img"
      aria-label={`${name} text fallback`}
    >
